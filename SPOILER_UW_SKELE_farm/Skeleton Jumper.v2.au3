...
Func SpikeSkele($SkeleID, $CallerID)
	Local $lDeadlock = TimerInit()
	Out("Spiking Skeleton")Func SkeleBoom()
    ...
    Do
        Sleep(200)
    Until DllStructGetData(GetAgentByID($lSkeleID), 'HP&apos') < .5 Or GetIsDead(-2) Or GetIsDead($lSkeleID)
...
EndFunc   ;==>SkeleBoom
	...
	;If DllStructGetData(GetAgentByID(-2), 'MaxEnergy') >= 40 Then
	UseSkill($Dash, -2)
	Sleep(100)
